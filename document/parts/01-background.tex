...
We assume some familiarity with some kind of dependent type theory, which
should be enough to understand our setting.
...
We will be considering the structural rules and some logical ones of
predicative Martin-L\"{o}f type theory \cite{ML84} as presented in \cite[App.\
A]{KL12}. Informally, the latter provide:
\begin{enumerate}
  \item \emph{dependent sums} $\Sigmas(A,B)$, also called \emph{$\Sigmas$-types}, and the associated introduction,
    elimination and computation rules;
  \item \emph{dependent products} $\Pis(A,B)$, also called \emph{$\Pis$-types}, and the associated introduction,
    elimination and computation rules;
  \item \emph{identity types} $\Ids_A$, also called \emph{$\Ids$-types}, and the associated introduction,
    elimination and computation rules.
\end{enumerate}
...
Theory \cite{Uni13}, where it is a consequence of univalence.
...
context. This is fine, since it means that our results will apply very
generally.
\end{rmk}
...
$C$\emph{-systems} \cite{Voe14a,Voe14b,Voe15a,Voe15b}. For informations on the
others we refer to the nLab \cite{nlab:categorical_model_of_dependent_types}.
...
\begin{rmk}
  ...
  an equivalent model of the same kind but with functorial pullbacks. Famously,
  we also have homotopical models of dependent type
  theories, like \emph{type-theoretic fibration categories} \cite{Shu14}, which
  however carry much more structure as they are meant to also model the logical
  rules.
\end{rmk}
...
\begin{construction}\cite[2.6]{Car78}
  ...
  \cite[App.\ A.1]{KL12}, its syntactic category $\Syn{\T}$ has:
  \begin{enumerate}
...
\begin{rmk}
  ...
  multi-sorted) essentially algebraic theory \cite{AR94} with sorts
...
\begin{exmp}
  ...
  categories was developed by Voevodsky in \cite{Voe14b} and subsequent papers,
  where he starts from a
  universe category, that is a quadruple given by a category $\cC$, a morphism
  $p\colon\tilde{U}\rightarrow U$, choices of pullback squares along $p$ for all
  morphisms to $U$ and a choice of a final object of $\cC$. His methods were
  used to construct a simplicial model of univalent type theory in \cite{KL12}.
\end{exmp}
...
theory in \cite[Ch.\ III, p.\ 181]{Str91} and a proof formalized in Agda
for predicative Martin-L\"{o}f is
currently available at \cite{Bru20}, however we still do not have a fully
...
Currently, researchers often assume this conjecture to be true because it
