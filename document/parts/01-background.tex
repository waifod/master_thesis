...
\begin{rmk}
  ...
  theories, like \emph{type-theoretic fibration categories} \cite{Shu14} and
  \emph{$\infty$-toposes} \cite{Shu19}, which however carry much more structure
  as they also model the logical rules.
\end{rmk}
...
\begin{notation}
  ...
  construct the $f_i$ from the variables in the domain context.
\end{notation}
...
\begin{construction}\label{applyf}
  ...
  $a\mapsto(a,\app(f,a))$, meaning that $\lambda_{A,p^*_AB}(1_A,f)$
  syntactically corresponds to $\lambda(a:A).\app(f,a):\Pis(A,B)$.
\end{construction}
