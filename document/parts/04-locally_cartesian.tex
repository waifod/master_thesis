...
\begin{rmk}
  ...
  structure on $\Syn{T}^{cxt}$ (where the condition on dependent projection
  ...
  argue in $\Syn{T}^{cxt}$ using dependent type theory.
\end{rmk}
...
which is exactly what we were looking for.
  ...
  Remark \ref{applyf}.
  \wfd{(USED INTERNAL REASONING AGAIN)} while the
...
\begin{thm}[\cite{Kap17}]\label{finalthm}
