...
will be our usage of extended logical structures. We also briefly mention the
\emph{Internal Languages Conjecture}.
...
\begin{thm}[{\cite[Thm.\ 9.3.17]{Kap14}}]\label{finalthm}
...
in Remark \ref{internal} we
referred to some results derived from it and our theorem, like the Internal
Languages Conjecture \cite[Conj.\ 3.7]{KL16}, which we now state and explain.
...
conjecture means that dependent type theories with the
specified logical rules can be modelled by $\infty$-categories with the
appropriate structures, exactly like Homotopy Type theory. There is currently an
ongoing project to 
...
elementary $\infty$-topos should be one which would make the
statement true.
...
are not simple context extensions. This was instrumental 
extensions
